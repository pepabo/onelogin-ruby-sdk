  ...
  class EnrollFactorRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OneLogin::EnrollFactorRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OneLogin::EnrollFactorRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'factor_id')
        self.factor_id = attributes[:'factor_id']
      end
    ...
    def list_invalid_properties
      ...
      if @factor_id.nil?
        invalid_properties.push('invalid value for "factor_id", factor_id cannot be nil.')
      end
    ...
    def hash
      [factor_id, display_name, expires_in, verified, redirect_to, custom_message].hash
    end
